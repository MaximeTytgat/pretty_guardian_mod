...
public class CuteAbstractArrow extends Projectile {
    private static final double ARROW_BASE_DAMAGE = 2.0D;
    private static final EntityDataAccessor<Byte> ID_FLAGS = SynchedEntityData.defineId(CuteAbstractArrow.class, EntityDataSerializers.BYTE);
    private static final EntityDataAccessor<Byte> PIERCE_LEVEL = SynchedEntityData.defineId(CuteAbstractArrow.class, EntityDataSerializers.BYTE);
    ...
    public CuteAbstractArrow.Pickup pickup = CuteAbstractArrow.Pickup.DISALLOWED;
    ...
    protected CuteAbstractArrow(EntityType<? extends CuteAbstractArrow> p_36721_, Level p_36722_) {
        super(p_36721_, p_36722_);
    }

    protected CuteAbstractArrow(EntityType<? extends CuteAbstractArrow> p_36711_, double p_36712_, double p_36713_, double p_36714_, Level p_36715_) {
        this(p_36711_, p_36715_);
        this.setPos(p_36712_, p_36713_, p_36714_);
    }

    protected CuteAbstractArrow(EntityType<? extends CuteAbstractArrow> p_36717_, LivingEntity p_36718_, Level p_36719_) {
        ...
        if (p_36718_ instanceof Player) {
            this.pickup = CuteAbstractArrow.Pickup.ALLOWED;
        }
    ...
    public void tick() {
        ...
        if (this.inGround && !flag) {
            ...
            if (this.isCritArrow()) {
                for(int i = 0; i < 4; ++i) {
                    CuteCore.LOGGER.info("Crit Arrow ...");
                    this.level().addParticle(ParticleTypes.HEART, this.getX() + d5 * (double)i / 4.0D, this.getY() + d6 * (double)i / 4.0D, this.getZ() + d1 * (double)i / 4.0D, -d5, -d6 + 0.2D, -d1);
                }
            }
            ...
            if (this.isInWater()) {
                for(int j = 0; j < 4; ++j) {
                    float f2 = 0.25F;
                    this.level().addParticle(ParticleTypes.HEART, d7 - d5 * 0.25D, d2 - d6 * 0.25D, d3 - d1 * 0.25D, d5, d6, d1);
                }
    ...
    protected void onHitEntity(EntityHitResult p_36757_) {
        ...
        if (entity.hurt(damagesource, (float)i)) {
            ...
            if (!this.level().isClientSide && this.getDeltaMovement().lengthSqr() < 1.0E-7D) {
                if (this.pickup == CuteAbstractArrow.Pickup.ALLOWED) {
    ...
    public void readAdditionalSaveData(CompoundTag p_36761_) {
        ...
        this.pickup = CuteAbstractArrow.Pickup.byOrdinal(p_36761_.getByte("pickup"));
    ...
    public void setOwner(@Nullable Entity p_36770_) {
        ...
        if (p_36770_ instanceof Player) {
            this.pickup = ((Player)p_36770_).getAbilities().instabuild ? CuteAbstractArrow.Pickup.CREATIVE_ONLY : CuteAbstractArrow.Pickup.ALLOWED;
        }
    ...
    public static enum Pickup {
        ...
        public static CuteAbstractArrow.Pickup byOrdinal(int p_36809_) {
