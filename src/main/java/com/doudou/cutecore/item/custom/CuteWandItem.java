...
public class CuteWandItem extends BowItem  {
    ...
    public CuteWandItem(Properties p_40660_) {
        super(p_40660_);
    }
            ...
            if (!itemstack.isEmpty() || flag) {
                ...
                if (!((double)f < 0.1D)) {
                    ...
                    if (!level.isClientSide) {
                        HeartItem arrowitem = (HeartItem) CuteCoreItem.CUTE_HEART.get();
                        HeartEntity abstractarrow = arrowitem.createArrow(level, itemstack, player);
                    ...
                    level.playSound((Player)null, player.getX(), player.getY(), player.getZ(), SoundEvents.AXE_STRIP, SoundSource.PLAYERS, 1.0F, 1.0F / (level.getRandom().nextFloat() * 0.4F + 1.2F) + f * 0.5F);
